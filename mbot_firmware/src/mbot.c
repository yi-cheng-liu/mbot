...
static float global_lastVelLeft=0;
static float global_lastVelRight=0;
...
void mbot_print_state(serial_mbot_imu_t imu, serial_mbot_encoders_t encoders, serial_pose2D_t odometry, serial_mbot_motor_vel_t motor_vel){
    ...
    const char* enc_headings[] = {"v_x", "v_y", "w"};
    ...
    char buf[1024] = {0};
    float imu_acc[3]={mbot_vel.vx,mbot_vel.vy,mbot_vel.wz};//has bias in ax,ay with 0.77,0.37 respectively
    float vel_cmd[3]={mbot_vel_cmd.vx,mbot_vel_cmd.vy,mbot_vel_cmd.wz};
    generateTableFloat(buf, 1, 3, "Bot_Vel", enc_headings, imu_acc);
    printf("\r%s", buf);
    ...
    buf[0] = '\0';
    generateTableFloat(buf, 1, 3, "MOTOR", motor_vel_headings, vel_cmd);
    printf("\r%s", buf);
...
bool mbot_loop(repeating_timer_t *rt)
{
    ...
    mbot_calculate_odometry(mbot_vel, MAIN_LOOP_PERIOD, mbot_imu, &mbot_odometry);
    ...
    if (global_comms_status == COMMS_OK)
    {
        if(drive_mode == MODE_MOTOR_VEL_OL){
        ...
        }else if(drive_mode == MODE_MBOT_VEL){//original MODE_MBOT_VEL
            ...
            if(MBOT_DRIVE_TYPE == OMNI_120_DRIVE){
                ...
                float kp1=0.6,kp2=0.6,ki1=0.2,ki2=0.2,kd1=0.2,kd2=0.2;
                //diff is integral error
                float diff1=mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_left]*params.wheel_radius+global_lastVelLeft-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                float diff2=-mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_right]*params.wheel_radius+global_lastVelRight-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                //e is differenciate of error
                float eleft=mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_left]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                float eright=-mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_right]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                if(mbot_vel_cmd.vx==0 && mbot_vel_cmd.wz==0){mbot_motor_vel_cmd.velocity[params.mot_left]=0;mbot_motor_vel_cmd.velocity[params.mot_right]=0;}
                else if (mbot_vel_cmd.vx==0 && mbot_vel_cmd.wz!=0){//pure rotation PID
                    kp1=0.6;kp2=0.6;ki1=0.1;ki2=0.1;
                    kd1=0.2;kd2=0.2;
                    diff1=global_lastVelLeft-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                    diff2=global_lastVelRight-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                    eleft=-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                    eright=-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS;
                    mbot_motor_vel_cmd.velocity[params.mot_left] = (kp1*(-mbot_motor_vel.velocity[params.mot_left]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS)+ki1*diff1+kd1*(eleft-lastLeft)) / DIFF_WHEEL_RADIUS;
                    mbot_motor_vel_cmd.velocity[params.mot_right] = (kp2*(-mbot_motor_vel.velocity[params.mot_right]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS)+ki2*diff2+kd2*(eright-lastRight)) / DIFF_WHEEL_RADIUS;
                    // mbot_motor_vel_cmd.velocity[params.mot_left] = (-mbot_vel_cmd.wz*DIFF_BASE_RADIUS) / DIFF_WHEEL_RADIUS;
                    // mbot_motor_vel_cmd.velocity[params.mot_right] = (mbot_vel_cmd.wz*DIFF_BASE_RADIUS) / DIFF_WHEEL_RADIUS;
                }
                else{//Forward PID
                    mbot_motor_vel_cmd.velocity[params.mot_left] = (kp1*(mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_left]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS)+ki1*diff1+kd1*(eleft-lastLeft)) / DIFF_WHEEL_RADIUS;
                    mbot_motor_vel_cmd.velocity[params.mot_right] = (kp2*(-mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_right]*params.wheel_radius-(mbot_vel_cmd.wz-mbot_vel.wz)*DIFF_BASE_RADIUS)+ki2*diff2+kd2*(eright-lastRight)) / DIFF_WHEEL_RADIUS;
                }
                // mbot_motor_vel_cmd.velocity[params.mot_left] = (kp1*(mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_left]*params.wheel_radius)) / DIFF_WHEEL_RADIUS;
                // mbot_motor_vel_cmd.velocity[params.mot_right] = (kp2*(-mbot_vel_cmd.vx-mbot_motor_vel.velocity[params.mot_right]*params.wheel_radius)) / DIFF_WHEEL_RADIUS;
                ...
                global_lastVelLeft=diff1;
                global_lastVelRight=diff2;
                lastLeft=eleft;
                lastRight=eright;
                float vel_left_comp = params.motor_polarity[params.mot_left] * mbot_motor_vel_cmd.velocity[params.mot_left];
                float vel_right_comp = params.motor_polarity[params.mot_right] * mbot_motor_vel_cmd.velocity[params.mot_right];
                ...
                mbot_motor_pwm_cmd.pwm[params.mot_right] = _calibrated_pwm_from_vel_cmd(vel_right_comp, params.mot_right);
                mbot_motor_pwm_cmd.pwm[params.mot_left] = _calibrated_pwm_from_vel_cmd(vel_left_comp, params.mot_left);
                // printf("right:%fleft:%f\n",mbot_motor_pwm_cmd.pwm[params.mot_right],mbot_motor_pwm_cmd.pwm[params.mot_left]);
            }
        }
        else {
            drive_mode = MODE_MOTOR_PWM;
