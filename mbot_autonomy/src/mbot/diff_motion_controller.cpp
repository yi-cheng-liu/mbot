...
private:
    float pid[3] = {0.7, 0.4, 0.}; //kp, ka, kb
    float d_end_crit = 0.02;
    float d_end_midsteps = 0.05;
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            float max_fwd=0.3;
            if (cmd.vx >max_fwd) cmd.vx = max_fwd;
            else if (cmd.vx < -max_fwd) cmd.vx = -max_fwd;

            // Angular vel
            float max_ang_vel = M_PI * 1.5 / 3.0;
