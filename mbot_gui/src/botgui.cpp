...
{
    // If a Ctrl + Left-click, send a path2D_t with a single position
    ...
    else if((event->button_mask & VX_BUTTON3_MASK)) // && (event->modifiers == 0)
    {
        ...
        planner.setMap(map_);
        path2D_t plannedPath = planner.planPath(slamPose_, target);
...
void BotGui::handlePose(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::pose2D_t* pose)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
...
void BotGui::handleOdometry(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::pose2D_t* odom)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
...
void BotGui::handlePath(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::path2D_t* path)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
    path_ = *path;
    havePath_ = true;
...
void BotGui::addPose(const mbot_lcm_msgs::pose2D_t& pose, const std::string& channel)
{
